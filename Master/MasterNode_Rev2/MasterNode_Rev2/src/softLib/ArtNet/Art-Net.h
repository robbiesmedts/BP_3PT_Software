//////////////////////////////////////////////////////////////
//
// Copyright Artistic Licence Holdings Ltd 1998-2018
// Author:	Wayne Howell
...
// updated 7/11/17 Dali sniffer added
// updated 1/3/19 ArtPoll targetting added
//
//////////////////////////////////////////////////////////////
...
#define OscPortListen 7000
#define OscPortTalk 8000
...
#define OpDnu1		    	0x2050 // Unauthorised use reported - will not be used in Art-Net
#define OpDnu2		    	0x2051 // Unauthorised use reported - will not be used in Art-Net
#define OpDnu3		    	0x2060 // Unauthorised use reported - will not be used in Art-Net

#define OpPollReply   	  	0x2100 // ArtPollReply
#define OpPollFpReply    	0x2200 // Additional advanced reply - deprecated
#define OpDiagData	    	0x2300 // Diagnostics data message
#define OpSnifDali	    	0x2302 // Sniffer packet for dali data
...
// bit masks for ArtPollReply.PortType field
#define PORTTYPE_OUTPUT_ARTNET			    0x80	// == %10000000	// Set if this channel can output data from the Art-Net network
#define PORTTYPE_INPUT_ARTNET			    0x40	// == %01000000	// Set if this channel can input onto the Art-Net network
#define PORTTYPE_PROTOCOL_DMX512		    0x00	// == %00000000	// DMX512
#define PORTTYPE_PROTOCOL_MIDI			    0x01	// == %00000001	// MIDI
#define PORTTYPE_PROTOCOL_AVAB			    0x02	// == %00000010	// Avab
#define PORTTYPE_PROTOCOL_COLORTRAN_CMX     0x03	// == %00000011	// Colortran CMX
#define PORTTYPE_PROTOCOL_ADB_625		    0x04	// == %00000100	// ADB 62.5
#define PORTTYPE_PROTOCOL_ARTNET		    0x05	// == %00000101	// Art-Net
#define PORTTYPE_PROTOCOL_DALI  		    0x06	// == %00000110	// DALI

// bit masks for ArtPollReply.GoodInput field
#define GOODINPUT_DATA_RECEIVED			    0x80		// Data received
#define GOODINPUT_DMX_TEST                  0x40        // Channel includes DMX512 test packets
#define GOODINPUT_DMX_SIP                   0x20        // Channel includes DMX512 SIP's
#define GOODINPUT_DMX_TEXT                  0x10        // Channel includes DMX512 text packets
#define GOODINPUT_INPUT_DISABLED		    0x08        // Input is disabled
#define GOODINPUT_RECEIVE_ERRORS		    0x04        // Receive errors detected

// bit masks for ArtPollReply.GoodOutput field
#define GOODOUTPUT_DATA_TRANSMITTED                 0x80    	// Data is being transmitted
#define GOODOUTPUT_DMX_TEST                         0x40        // Channel includes DMX512 test packets
#define GOODOUTPUT_DMX_SIP                          0x20        // Channel includes DMX512 SIP's
#define GOODOUTPUT_DMX_TEXT                         0x10        // Channel includes DMX512 text packets
#define GOODOUTPUT_ARTNET_MERGE                     0x08    	// Output is merging ArtNet data
#define GOODOUTPUT_DMX_OUTPUT_SHORT                 0x04    	// DMX output short detected on power up
#define GOODOUTPUT_MERGE_LTP                        0x02    	// Merge Mode is LTP
...
#define AcDirectionTx0  0x20    // Set Port 0 direction to Tx.
#define AcDirectionTx1  0x21    // Set Port 1 direction to Tx.
#define AcDirectionTx2  0x22    // Set Port 2 direction to Tx.
#define AcDirectionTx3  0x23    // Set Port 3 direction to Tx.

#define AcDirectionRx0  0x30    // Set Port 0 direction to Rx.
#define AcDirectionRx1  0x31    // Set Port 1 direction to Rx.
#define AcDirectionRx2  0x32    // Set Port 2 direction to Rx.
#define AcDirectionRx3  0x33    // Set Port 3 direction to Rx.
...
typedef struct S_ArtPoll
	...
	uchar Flags;
		...
		// bit 1 = 1 Node sends ArtPollReply when it needs to
		...
		// bit 5 = 1 Only reply if device has a Port-Address that is
		//				inclusively in the range TargetPortAddressBottom to
		//				TargetPortAddressTop.


	uchar DiagPriority;			// Set the lowest priority of diagnostics message that node should send. See DpXxx defines above.
	uchar TargetPortAddressTopHi;
	uchar TargetPortAddressTopLo;
	uchar TargetPortAddressBottomHi;
	uchar TargetPortAddressBottomLo;
	} T_ArtPoll, *p_T_ArtPoll;
...
typedef struct S_ArtPollReply
	...
	uchar AcnPriority; 	// sACN Priority for sACN generated by this node
	// output channel
	...
	uchar Status3;            // bit 76 = 00 If network data is lost, it will hold last state
							  ...
							  // bit 4 = 1 Node supports LLRP (Low Level Recovery Protocol

							  // bit 3 = 1 Outputs can be switched to an input
							  // bit 3 = 0 Outputs cannot be switched to an input
							  //  PortTypes[] defines the current direction of the port

							  // bit 2-0 = Transmit zero, receivers do not tests

	T_Uid DefaultUidResponder;    //RDMnet & LLRP UID

	uchar Filler[15]; // Filler bytes, currently zero.

	} T_ArtPollReply, *p_T_ArtPollReply;
...
typedef struct S_ArtDiagData
	...
	uchar ProtVerLo; 	// protocol version, set to ProtocolVersion
	uchar Filler1;
	uchar DiagPriority; 	// The message priority, see DpXxxx defines above.
	uchar LogicalPort;  // Zero for general messages, 1-255 if message relates to a logical port
	uchar Filler3;
	ushort Length;             // BYTE-SWAPPED MANUALLY. Length of array below
...
typedef struct S_ArtAddress
	...
	uchar AcnPriority; // sACN priority 0 - 200 (255 = ignore this)


	uchar Command; // see Acxx definition list

	} T_ArtAddress, *p_T_ArtAddress;
...
typedef struct S_ArtIpProg
	...
	uchar Command;   // Bit fields as follows: (Set to zero to poll for IP info)
		...
		// Bit 4 hi = Use custom default dateway in this packet.
		// Bit 3 hi = Return all three paraameters to default.
	...
	uchar ProgPortHi; 	// Deprecated
	uchar ProgPortLo;

	uchar ProgDgHi; 	// Use this default gateway if Command.Bit4
	uchar ProgDg2;
	uchar ProgDg1;
	uchar ProgDgLo;

	uchar Spare5; // Set to zero, do not test in receiver.
	uchar Spare6;
	uchar Spare7;
	uchar Spare8;
...
typedef struct S_ArtIpProgReply
	...
	uchar ProgDgHi; 	// The Node's current default dateway
	uchar ProgDg2;
	uchar ProgDg1;
	uchar ProgDgLo;
	uchar Spare7;
...
typedef struct  {
	uchar   ID[8];                    	// protocol ID = "Art-Net"
	ushort  OpCode;                  	// == OpSnifDali
	uchar   ProtVerHi;                	// 0
	uchar   ProtVerLo;                	// protocol version, set to ProtocolVersion
	uchar   Filler1;                  	// Transmit as zero
	uchar   Filler2;                  	// Transmit as zero

	uchar   BindIndex;              	// Set to zero if no binding, otherwise this number represents the order of bound devices. A lower number means closer to root device. Values of 0 and 1 mean Root
	uchar   Port;                   	// 0 - 3 to indicate the physical port
	uchar   DataBackward;           	// Dali backward data
    uchar   UserFlags;

										//Bit 0 = 1 = Forward data in this packet is valid
										//Bit 1 = 1 = Backward data in this packet is valid
										//Bit 2 = 1 = The time delay between forward and backward data is within specification
										//Bit 3 = 1 = The device that originated this packet also transmitted the forward frame
										//Bit 4 = 1 = A transmission error occurred.
										//Bit 5 = 1 = Backward data was received at an unexpected time
										//Bit 6 = 1 = Receive error
										//Bit 7 = 1 = Transmit this packet to DALI port (Used to inject diagnostic messages into DALI stream).

	uchar   DataForwardHi;              //The address bits of the forward data
    uchar   DataForwardLo;              //The command bits of the forward data
